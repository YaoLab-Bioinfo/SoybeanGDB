...
ld.heatmapw <- function(chr="chr15", start=1000000, end=1011111, snp.pos=c(1), 
  ...
  dat <- fetchSnpAllelew(chr=chr, start=start, end=end, accession=accession, mutType=mutType)
  ...
  if (gene) {
    ...
    p1 <- geneStruw(chr=chr, start=start, end=end)
