...
nucDivw <- function(chr="chr15", nuc.start=1000000, nuc.end=1011111, step=20, 
                   groups=c("Glycine soja", "Landraces and elites"), numerator="Glycine soja", denominator="Landraces and elites",
                   mutType = NULL) {
  
  if ( exists("fetchSnpAllelew") ){
  }else{
    source("fetchSnpAllelew.R")
  }
  #library(pegas)
  data <- fetchSnpAllelew(chr=chr, start=nuc.start, end=nuc.end, mutType = mutType)[[1]]
  
  gene_info_s$start <- as.numeric(gene_info_s$start)
  gene_info_s$end <- as.numeric(gene_info_s$end)
  data.L <- gsub("/.+", "", data)
  data.R <- gsub(".+/", "", data)
  ...
  div.group <- lapply(unique(c(groups, numerator, denominator)), function(x){
    x <- gsub(" ", "_", x)
    x.accession <- readLines(paste0("./data/w82Rdata/", x, ".soya_82.txt"))
  ...
  diVTxt82 <<- div.group.df
  diVTxt82 <<- diVTxt82[, c(ncol(diVTxt82), 1:(ncol(diVTxt82)-1))]
  names(diVTxt82)[1] <<- "position"
  ...
  div.group.df.1.long$group <- factor(div.group.df.1.long$group, levels = c("Glycine soja", "Landraces and elites")[c("Glycine soja", "Landraces and elites") %in% unique(div.group.df.1.long$group)] )
